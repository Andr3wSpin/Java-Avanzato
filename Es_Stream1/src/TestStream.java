...
//        stampaCollection(elenco);
        ...
        //aggiorno pernottamento
        List<Agriturismo> aggiornaPernottamento = elenco.stream().filter(f -> f.getPostiLetto() > 0).
                map(agriturismo -> {
        ...
        Optional<String> ComuneMaxPosti = campeggi.stream().filter(f -> f.isCamping() == true)
                .reduce((agriturismo, agriturismo2) ->
